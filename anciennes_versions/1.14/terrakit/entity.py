...
class Entity:
    ...
    def is_valid(self): 
        if self.world:
            if self.world.contains_entity(self):
                return True
        return False
...
class Zombie(Mob):
    DETECTION_DISTANCE = game_property.TILE_SIZE * 10
...
class Skeleton(Mob):
    ...
    def update_texture(self):
        self.head_texture = context.get_resource_pack().texture_manager().get_texture(TextureType.SKELETON_HEAD)
        self.head_texture = pygame.transform.scale(self.head_texture, (self.rect.width- 4, self.rect.width - 4))

        self.body_texture = context.get_resource_pack().texture_manager().get_texture(TextureType.SKELETON_BODY)
        self.body_texture = pygame.transform.scale(self.body_texture, (self.rect.width, self.rect.width))

        self.leg_texture = context.get_resource_pack().texture_manager().get_texture(TextureType.SKELETON_LEG)
        self.leg_texture = pygame.transform.scale(self.leg_texture, (self.rect.width - 4, self.rect.width))
        
        self.arm_texture = context.get_resource_pack().texture_manager().get_texture(TextureType.SKELETON_ARM)
        self.arm_texture = pygame.transform.scale(self.arm_texture, (self.rect.width // 2, self.rect.width))
    ...
    def __str__(self):
        return f"Skeleton(pos:{self.get_pos_tile()})"
...
class Alien(Mob):
    ...
    def to_json(self, type_="Alien", dict_=None):
        if dict_ is None:
            dict_ = {}
    ...
    def __str__(self):
        return f"Alien(pos:{self.get_pos_tile()})"
...
class Gnome(Mob):
    ...
    def to_json(self, type_="Gnome", dict_=None):
        if dict_ is None:
            dict_ = {}
    ...
    def __str__(self):
        return f"Gnome(pos:{self.get_pos_tile()})"
