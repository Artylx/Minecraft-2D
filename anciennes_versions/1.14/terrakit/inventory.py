...
class Inventory():
    ...
    def can_pickup(self, stack):
        ...
        # 1. Vérifier les stacks existants
        for i in range(self.size):
            slot = self.get_slot(i)
            item = slot.get()
            ...
            if item.item_property == stack.item_property:
                max_stack = item.item_property.max_stack
                free_space = max_stack - item.count
        ...
        # 2. Vérifier s'il reste un slot vide
        for i in range(self.size):
